...
public class ValidationItemControllerV4 {
    ...
    public String addItem(@Validated @ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) { // 기존코드에서 Model이 추가됨
        ...
        if(bindingResult.hasErrors()){ // 에러가 존재하면
            ...
            return "validation/v4/addForm";// 다시 입력폼으로 이동
        }
    ...
    @PostMapping("/add")
    public String addItem(@Validated @ModelAttribute("item") ItemSaveForm form, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) { // 기존코드에서 Model이 추가됨
        //특정 필드 예외가 아닌 전체 예외
